  ...
  return (
    <Card className="max-w-4xl mx-auto mt-6">
      <CardHeader>
        ...
        <CardDescription>
          Saved color suggestions and outfit plans
        </CardDescription>
      </CardHeader>
      <CardContent>
            ...
            <div key={p.id} className="rounded-lg border p-4">
              ...
              <div className="space-y-2">
                ...
                {p.richMatches && (
                  <>
                    <Row
                      label="Analogous"
                      colors={p.richMatches.analogous || []}
                    />
                    <Row
                      label="Complementary"
                      colors={[p.richMatches.complementary].filter(Boolean)}
                    />
                    <Row
                      label="Split Complementary"
                      colors={p.richMatches.splitComplementary || []}
                    />
                    <Row
                      label="Triadic"
                      colors={p.richMatches.triadic || []}
                    />
                    <Row
                      label="Tetradic"
                      colors={p.richMatches.tetradic || []}
                    />
                    <Row
                      label="Monochrome"
                      colors={p.richMatches.monochrome || []}
                    />
                    <Row
                      label="Neutrals"
                      colors={p.richMatches.neutrals || []}
                    />
                  </>
                )}
              </div>
              {p.plan && (
                <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-2 mt-3">
                  {(['top','bottom','outerwear','footwear','accessory'] as const).map(
                    (k) => (
                      <div
                        key={k}
                      ...
                      >
                        ...
                        <div
                          ...
                          style={{
                            backgroundColor:
                              (p.plan as any)[k] || '#f3f4f6'
                          }}
                          title={(p.plan as any)[k] || ''}
                        />
                      </div>
                    )
                  )}
